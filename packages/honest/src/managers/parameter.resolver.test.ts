...
import type { IMetadataRepository, ParameterMetadata } from '../interfaces';
...
const liveMetadataRepository: IMetadataRepository = {
  ...
  getControllerComponents: (type, controller) =>
    MetadataRegistry.getController(type, controller) as any,
  ...
  test('emits debug warning for sparse decorated indices', async () => {
    ...
    expect(
      logs.some(
        (log) =>
          log.level === 'warn' &&
          log.context?.category === 'pipeline' &&
          log.message.includes('Potential parameter binding mismatch') &&
          (log.context as Record<string, unknown>).handlerArity === 3,
      ),
    ).toBe(true);
  });
    ...
    expect(
      logs.some(
        (log) =>
          log.level === 'warn' &&
          log.context?.category === 'pipeline' &&
          (log.context as Record<string, unknown>).maxDecoratorIndex === 2,
      ),
    ).toBe(true);
  });
});
