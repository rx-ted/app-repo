...
import type { ApiDocOptions } from './controller-options.interface';
...
export interface HttpMethodOptions {
  ...
  /**
   * API documentation options for this route
   * Only used when API doc plugin is registered
   *
   * @example
   * ```typescript
   * @Get('users', {
   ...
   *     tags: ['User'],
   *     responses: { 200: { description: 'Success', schema: UserSchema } }
   *   }
   * })
   * getUsers() { ... }
   * ```
