...
export function createErrorResponse(
  ...
  if (normalizedException instanceof HTTPException) {
    ...
    return {
      response,
      status: (options?.status || normalizedException.status) as ContentfulStatusCode,
    };
  }
  ...
  const response: ErrorResponse = {
    ...
    message:
      options?.title ||
      (env.mode === 'prod' ? 'Internal Server Error' : normalizedException.message),
    timestamp,
    path,
    requestId,
    code: options?.code || normalizedException.name,
    details:
      options?.additionalDetails ||
      (env.mode === 'dev' ? { stack: normalizedException.stack } : undefined),
    ...(options?.detail && { detail: options.detail }),
  };
