...
describe('MetadataRegistry', () => {
  describe('routes', () => {
    ...
    test('setRoutes overwrites existing routes', () => {
      MetadataRegistry.addRoute(FakeController, {
        path: '/old',
        method: 'GET',
        handlerName: 'old',
      });
  ...
  describe('parameters', () => {
    test('setParameterMap / getParameters round-trips', () => {
      const params = new Map([
        [
          'index',
          [{ index: 0, name: 'body', data: undefined, factory: () => {}, metatype: String }],
        ],
      ]);
  ...
  describe('component registration — handler level', () => {
    test('registerHandler / getHandler round-trips', () => {
      MetadataRegistry.registerHandler(
        'middleware',
        FakeController,
        'index',
        fakeMiddleware as any,
      );
      expect(MetadataRegistry.getHandler('middleware', FakeController, 'index')).toEqual([
        fakeMiddleware,
      ]);
    });
