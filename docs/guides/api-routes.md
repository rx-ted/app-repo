...
**基路径：** `/api/v1/versions`  
**守卫：** 列表和详情公开，增删改需 `admin` + `version:access:any`
...
| 参数       | 类型   | 默认值 | 说明                                  |
| ---------- | ------ | ------ | ------------------------------------- |
| scope      | string | "all"  | 检测范围：`schema` \| `code` \| `all` |
...
```
POST /api/v1/versions/detect
```

**认证：** `admin` + `version:access:any`
...
| moduleName | string | -      | 按模块过滤                                   |
| changeType | string | -      | 变更类型                                     |
| status     | string | -      | 状态：`draft` \| `released` \| `rolled_back` |
...
```json
{
  ...
  "title": "string (1-200)",
  "changeType": "feature | bugfix | improvement | breaking | data_change | code_change",
  "moduleName": "string (最多 100)",          // 可选
  "summary": "string",                        // 可选
  "details": { ... },                         // 可选
  "author": "string (最多 100)"               // 可选
}
```
...
```
PUT /api/v1/versions/:id/release
```
...
**认证：** `admin` + `metrics:access:any`

**响应：** `any`

### 17.2 获取 Prometheus 格式指标
